...
2.  **Define Viewports**:
    - **Mobile Integration**: 375x812 (iPhone X/12/13 mini range) - _Critical for responsiveness_
    - **Desktop Standard**: 1440x900 (Common laptop) - _Critical for layout stability_
    - **Tablet/Narrow**: 768x1024 (iPad vertical) - _Check for awkward wrapping_
...
_Check these on every page you visit._
...
_Reference the `style-audit` skill for canonical values._
...
2.  **Apply Fix**:
    - _Spacing_: formatting `p-4` -> `p-5`?
    - _Layout_: `flex-row` -> `flex-col md:flex-row`?
    - _Contrast_: `text-gray-400` -> `text-muted-foreground`?
