---
description: Meta-skill that guides AI agents through a rigorous process for creating high-quality workflow and skill files
---

# Skill Creator

A meta-workflow for producing other `.agent/workflows/` skill files. A poorly written skill produces poor results at scale — every ambiguity compounds across every invocation. This skill enforces a rigorous creation process so that every new workflow meets a high quality bar. The deliverable is a single Markdown file with YAML frontmatter, placed in `.agent/workflows/`.

## When to Use This Workflow

- User asks to create a new workflow or skill file
...
- Exactly one field: `description`
- Under 120 characters
- Format: "[Verb phrase] for [domain/context]"
...
5. **Check for overlap** — Read all files in `.agent/workflows/` to avoid duplicating existing skills. Reference rather than repeat.
...
1. **Write frontmatter and title** — Get the description right first. If you cannot summarize the skill in one sentence, the scope is wrong. Go back to Phase 1.
...
1. **Name the file** — Use kebab-case: `.agent/workflows/[skill-name].md`. Name should be 2-4 words communicating the domain.
2. **Save the file** — Write to `.agent/workflows/[skill-name].md`.
3. **Verify frontmatter** — Confirm the YAML parses correctly (single `description` field).
4. **Cross-reference check** — If the skill references other skills, verify those paths exist.
5. **Inform the user** — Summarize what was created, its purpose, and how to invoke it.
...
- **Overlap check:** No existing review skill in `.agent/workflows/`
...
File saved as `.agent/workflows/code-review.md`. User notified with summary.
...
- [ ] File named in kebab-case: `[skill-name].md`
- [ ] File placed in `.agent/workflows/`
...
2. **Steal structure from existing skills** — Read `.agent/workflows/feature_implementation_guide.md` and `.agent/workflows/ui-ux-pro-max.md` before starting. Match the formatting conventions already established.
