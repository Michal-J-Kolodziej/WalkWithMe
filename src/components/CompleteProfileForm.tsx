...
import { ArrowRight, Calendar, Loader2, MapPin, Sparkles, UserCircle } from "lucide-react"
...
export function CompleteProfileForm() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : t('completeProfile.error', 'Failed to complete profile')
      setError(message)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">{t('completeProfile.settingUp', 'Setting up your account...')}</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background px-4">
        <div className="text-center space-y-6 max-w-md">
          ...
          <h1 className="text-2xl font-bold text-foreground">{t('completeProfile.sessionExpired', 'Session Expired')}</h1>
          <p className="text-muted-foreground">
            {t('completeProfile.sessionExpiredDesc', 'Your authentication session could not be verified. Please log in again to complete your profile.')}
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="space-y-6">
                ...
                <div className="space-y-3">
                    <Label className="text-lg font-medium ml-1">{t('profile.role')}</Label>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-2 group">
                        <Label htmlFor="location" className="text-base font-medium ml-1 transition-colors group-focus-within:text-primary">
                    ...
                    <div className="space-y-2 group">
                        <Label htmlFor="bio" className="text-base font-medium ml-1 transition-colors group-focus-within:text-primary">
                            {t('completeProfile.bio')}
                        </Label>
                        <div className="relative">
                            <UserCircle className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground group-focus-within:text-primary transition-colors" size={20} />
                            <Input 
                                id="bio" 
                                name="bio" 
                ...
                {/* Age Field */}
                <div className="space-y-2 group">
                    <Label htmlFor="age" className="text-base font-medium ml-1 transition-colors group-focus-within:text-primary">
                ...
                <span className="relative z-10 flex items-center justify-center gap-2">
                    {isLoading ? (
                      <>
                        ...
                        <>{t('completeProfile.continue')} <ArrowRight className="group-hover:translate-x-1 transition-transform" /></>
                    )}
                </span>
