...
const SPOT_TYPES: Array<{ type: SpotType; label: string; color: string }> = [
...
export function SpotsMap() {
  ...
  const [activeFilters, setActiveFilters] = useState<Array<SpotType>>([
    'park',
    'vet',
    'store',
    'cafe',
        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                size="lg"
                className="rounded-full h-12 w-12 shadow-lg bg-background hover:bg-muted hover:scale-110 active:scale-95 transition-all duration-300 group border border-border p-0 flex items-center justify-center"
                onClick={handleLocateMe}
                disabled={!userLocation.latitude || !userLocation.longitude}
              >
                <Locate className="w-5 h-5 text-foreground group-hover:text-primary transition-colors" />
              </Button>
            </TooltipTrigger>
            <TooltipContent
              side="left"
