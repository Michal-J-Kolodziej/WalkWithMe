          ...
          <div className="p-6 pb-4 relative z-10">
            <SheetHeader className="text-left space-y-3">
              <div className="flex items-start justify-between gap-4">
                ...
                <Badge
                  variant="outline"
                  className="capitalize px-3 py-1 bg-background/50 backdrop-blur-md border-primary/20 text-primary animate-in fade-in zoom-in duration-300"
                >
                  {details.type}
                </Badge>
              </div>
            ...
            <div className="mt-6 flex gap-3">
              <Button
                className="flex-1 shadow-lg shadow-primary/10 transition-all hover:scale-[1.02]"
                size="default"
                onClick={() =>
                  window.open(
                    `https://www.google.com/maps/dir/?api=1&destination=${details.location.lat},${details.location.lng}`,
                  )
                }
              >
          ...
          {isAddingReview && (
            <div className="mb-8 bg-card p-5 rounded-2xl border border-border shadow-sm animate-in slide-in-from-top-4 duration-300">
              <h4 className="font-semibold mb-4 flex items-center gap-2">
                <MessageSquare className="w-4 h-4 text-primary" /> Write a
                Review
              </h4>
            ...
            <div className="space-y-4">
              {details.reviews?.length === 0 ? (
                ...
                details.reviews?.map((review: any) => (
                  <div
                    key={review._id}
                    className="flex gap-4 p-4 rounded-2xl bg-card/40 border border-border/40 hover:bg-card/60 transition-colors"
                  >
